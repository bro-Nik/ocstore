...
class URLify
{
     ...
     * @return string
	 */
	public static function filter ($text, $seogen_razdelitel, $length = 100, $language = "", $file_name = false, $use_remove_list = true, $lower_case = true, $treat_underscore_as_space = true)
    {
		...
		if ($treat_underscore_as_space) {
		    	$text = str_replace ($seogen_razdelitel, ' ', $text);             // treat underscores as spaces
		}
		$text = preg_replace ('/^\s+|\s+$/u', '', $text);  // trim leading/trailing spaces
		$text = preg_replace ('/[-\s]+/u', $seogen_razdelitel, $text);    // convert spaces to hyphens
		if ($lower_case) {
			$text = strtolower ($text);                        // convert to lowercase
		}
		return trim (substr ($text, 0, $length), $seogen_razdelitel);     // trim to first $length chars
	}
	/**
