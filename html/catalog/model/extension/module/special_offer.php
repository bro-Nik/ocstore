...
class ModelExtensionModuleSpecialOffer extends Model {
	public function getSpecialOffer($special_offer_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "special_offer s LEFT JOIN " . DB_PREFIX . "special_offer_description sd ON (s.special_offer_id = sd.special_offer_id) WHERE s.special_offer_id = '" . (int)$special_offer_id . "' AND sd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
	...
	public function getSpecialOfferInfoBySpecID($product_special_id) {		// Special Offer Info by product_special_id
		$query = $this->db->query("SELECT ps.special_offer_id, ps.price, ps.product_special_id, ps.date_start, ps.date_end, so.offer_type, so.image_label, so.gift_product_id, so.timer_status, so.cycle_of_timer FROM " . DB_PREFIX . "product_special ps LEFT JOIN " . DB_PREFIX . "special_offer so ON (ps.special_offer_id = so.special_offer_id) WHERE ps.product_special_id = '" . (int)$product_special_id . "'");
		...
		if ($query->num_rows) {
			...
			return array(
				...
				'timer_status'			=> isset($query->row['timer_status']) ? $query->row['timer_status'] : 1,
	...
	public function getSpecialOffers($data = array()) {
		$sql = "SELECT * FROM " . DB_PREFIX . "special_offer s LEFT JOIN " . DB_PREFIX . "special_offer_description sd ON (s.special_offer_id = sd.special_offer_id) WHERE sd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND s.list_customer_group_id LIKE '%" . $this->config->get('config_customer_group_id') . "%' AND s.offer_status = '1'";
 
		if (isset($data['show_ended']) && ($data['show_ended'] == '0')) {
			$sql .= " AND (s.date_end = '0000-00-00' OR s.date_end > NOW())";
		}	
		
		$sql .= " ORDER BY s.date_start ASC";
	...
	public function getCategoriesInSpecials($special_offer_id=0) {
		$sql = "SELECT cd.name, p2c.category_id AS category_id FROM " . DB_PREFIX . "product_to_category p2c LEFT JOIN " . DB_PREFIX . "category_description cd ON (p2c.category_id = cd.category_id) WHERE cd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p2c.product_id IN (SELECT ps.product_id FROM " . DB_PREFIX . "product_special ps WHERE ps.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW()))";
	...
	public function getProductsBySpecialOfferId($special_offer_id, $data = array()) {
		$sql = "SELECT DISTINCT ps.product_id, (SELECT AVG(rating) FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = ps.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating FROM " . DB_PREFIX . "product_special ps LEFT JOIN " . DB_PREFIX . "product p ON (ps.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND ps.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ps.special_offer_id = '" . (int)$special_offer_id . "'";

		if (!empty($data['filter_category_id'])) {
				$sql .= " AND " . (int)$data['filter_category_id'] . " IN (SELECT category_id FROM " . DB_PREFIX . "product_to_category p2c WHERE p2c.product_id = ps.product_id)";
		}	
	...
	public function getTotalProductsBySpecialOfferId($special_offer_id, $data = array()) {
