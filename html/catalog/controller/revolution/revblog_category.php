...
class ControllerRevolutionRevblogCategory extends Controller {
	public function index() {
		...
		$limit2 = $this->config->get('theme_' . $this->config->get('config_theme') . '_product_limit');
		...
		if ($category_info) {
			...
			foreach ($results as $result) {
				...
				$data['blogs'][] = array(
					...
					'data_added'  		=> isset($setting2['category_blog_grid']) && $setting2['category_blog_grid'] ? date('d.m', strtotime($result['date_available'])) : date($this->language->get('date_format_short'), strtotime($result['date_available']))
				);
			}
