...
class ControllerRevolutionRevpopupphone extends Controller {
	...
	public function make_order_phone() {
		...
		if ($this->config->get('captcha_' . $this->config->get('config_captcha') . '_status') && in_array('contact', (array)$this->config->get('config_captcha_page'))) {
			...
			if ($captcha) {
				$json['error']['field']['captcha'] = $captcha;
			}
		}

		$popupphone1 = isset($this->config->get('revtheme_header_popupphone')['status']) ? $this->config->get('revtheme_header_popupphone')['status'] : false;
		$popupphone2 = isset($this->config->get('revtheme_footer_all')['popup_phone']) ? $this->config->get('revtheme_footer_all')['popup_phone'] : false;
		$popupphone3 = isset($this->config->get('revtheme_footer_all')['callbtn']) ? $this->config->get('revtheme_footer_all')['callbtn'] : false;

		if (!isset($json['error']) && ($popupphone1 || $popupphone2 || $popupphone3)) {
