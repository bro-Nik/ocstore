...
class ControllerCatalogReviewload extends Controller {
	private $error = array();
	...
	public function index() {
		...
		if ( ! empty( $_FILES['file']['size'] ) ) {
			...
			/*	if ( ! in_array( $ext, $array_exel ) ) {
					echo 'Error: Invalid file type.';
				} else {*/

					$key       = rand( 11111, 99999 );
					$name_file = "stock" . ".$ext";
					$image     = DIR_UPLOAD."$name_file";
					//$image = "test/$name_file";
		...
		require_once DIR_SYSTEM.'library/PHPExcel.php'; //подключаем наш фреймворк
		$filepath      = $image;
		...
		for ( $row =1; ( $row <= $highestRow ); $row ++ ) {
			$otziv[] = array('art'=>trim( $objWorksheet->getCell( "T" . $row )->getValue()),
				'name'=>trim( $objWorksheet->getCell( "C" . $row )->getValue()),
				'date'=>str_replace(".","-",trim( $objWorksheet->getCell( "D" . $row )->getValue() )).' 00:00:00',
				'dost'=>trim( $objWorksheet->getCell( "O" . $row )->getValue() ),
				'ndost'=>trim( $objWorksheet->getCell( "P" . $row )->getValue() ),
				'koment'=>trim( $objWorksheet->getCell( "Q" . $row )->getValue() ),
				'drug'=>trim( $objWorksheet->getCell( "R" . $row )->getValue() ),
				'reit'=>trim( $objWorksheet->getCell( "L" . $row )->getValue() )
			);
		}
		...
		if ( ! empty( $otziv ) ) {
			foreach ( $otziv AS $o ) {
				$q = $this->db->query( "SELECT * FROM `oc_product` WHERE `sku` = '" . $o['art'] . "' LIMIT 1" );
				if ( $q->num_rows != 0 ) {
				$row = $q->row;


				$this->db->query("INSERT INTO `oc_review` (`product_id`, `customer_id`, `author`,
 `text`, `answer`, `answer_from`, `rating`, `status`, `date_added`,
  `date_modified`, `plus`, `minus`, `videos`, `images`, `bonus`, `likes`, `dislikes`, `purchased`, `admin_reply`, `admin_name`)
VALUES ('".$row['product_id']."', '0', '".$this->db->escape($o['name'])."',
 '".$this->db->escape($o['koment'])."', '', '', '".$this->db->escape($o['reit'])."', '1', '".$this->db->escape($o['date'])."'
 , '', '".$this->db->escape($o['dost'])."', '".$this->db->escape($o['ndost'])."', NULL, NULL, '0', '0', '0', '0', NULL, NULL);");
					$addcount++;
				}
			}
			$this->session->data['success'] = 'Добавили '.$addcount.' отзывов';
		}
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			'text' => 'Загрузка отзывов из файла',
			'href' => $this->url->link('catalog/reviewload', 'user_token=' . $this->session->data['user_token'] . $url, true)
		);

		$data['load'] = $this->url->link('catalog/reviewload/index', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		$this->response->setOutput($this->load->view('catalog/review_load', $data));
	}
