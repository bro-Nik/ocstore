...
class ModelRevolutionRevtabs extends Model {
	public function addtabs($data){
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "tabs SET sort_order = '" . (int)$data['sort_order'] . "', status = '".(int)$data['status']."', products = '".$data['tabs_product']."', categories = '".$data['tabs_category']."', manufactures = '".$data['tabs_manufacturer']."', ingore_products = '".$data['tabs_ingore_product']."', allproducts = '".(int)$data['allproducts']."', date_added = NOW()");
		
		$tab_id = $this->db->getLastId();
		
		if (isset($data['tabs_store'])) {
			foreach ($data['tabs_store'] as $store_id) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "tabs_to_store SET tab_id = '" . (int)$tab_id . "', store_id = '" . (int)$store_id . "'");
			}
		}
	...
	public function edittabs($tab_id, $data){
		...
		$this->db->query("UPDATE " . DB_PREFIX . "tabs SET sort_order = '" . (int)$data['sort_order'] . "', status = '".(int)$data['status']."', products = '".$data['tabs_product']."', categories = '".$data['tabs_category']."', manufactures = '".$data['tabs_manufacturer']."', ingore_products = '".$data['tabs_ingore_product']."', allproducts = '".(int)$data['allproducts']."' WHERE tab_id = '".(int)$tab_id."'");
		
		$this->db->query("DELETE FROM " . DB_PREFIX . "tabs_to_store WHERE tab_id = '" . (int)$tab_id . "'");
	...
	public function getTabsStores($tab_id) {
		$tab_store_data = array();

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "tabs_to_store WHERE tab_id = '" . (int)$tab_id . "'");
