...
class ModelExtensionShippingSpecialFree extends Model {
	function getQuote($address) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('specialfree_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");

		if (!$this->config->get('specialfree_geo_zone_id')) {
			$status = true;
		...
		if ($this->cart->getSubTotal() < $this->config->get('specialfree_total')) {
			$status = false;
		}
		...
		foreach ($products as $product) {
			...
			if (!empty($special_offer_id)) {
				...
				if ($special_offer_info['offer_type'] == 2 || ($special_offer_info['free_shipping'] && ($special_offer_info['offer_type'] == 1 || $special_offer_info['offer_type'] == 4 || $special_offer_info['offer_type'] == 5))) {
					if (array_key_exists($special_offer_id, $ship_offers)) { 
						$ship_offers[$special_offer_id]['qty'] += $product['quantity'];
						$ship_offers[$special_offer_id]['sum'] += $product['total'];
					} else {
						$ship_offers[$special_offer_id]['qty'] = $product['quantity'];
						$ship_offers[$special_offer_id]['sum'] = $product['total'];
						...
						$ship_offers[$special_offer_id]['product_sum'] = $special_offer_info['product_sum'];
		...
		foreach ($ship_offers as $ship_offer) {
			if ($ship_offer['qty']>=$ship_offer['product_quantity'] && $ship_offer['sum']>=$ship_offer['product_sum']) {
				$special_status = true;
		...
		if ($status) {
			...
			$description = $this->config->get('specialfree_description');
			...
			$method_data = array(
				...
				'quote'      => $quote_data,
				'sort_order' => $this->config->get('specialfree_sort_order'),
				'error'      => false
			);
		}
