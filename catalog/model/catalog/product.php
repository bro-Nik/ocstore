...
class ModelCatalogProduct extends Model {
	...
	public function getProductsByIds($data = array()) {
    	...
    	// 
    	$sql = "SELECT p.product_id, p.price, p.quantity > 0 as nalichie, p.price > 0 as nalichie2, (SELECT AVG(rating) AS total FROM " . DB_PREFIX . "review r1 
    	...
    	// Сохраняем порядок товаров как в исходном списке ID
    	if (empty($data['sort'])) {
        	$ordered_products = array();
        	foreach ($product_ids as $product_id) {
            	if (isset($product_data[$product_id])) {
                	$ordered_products[$product_id] = $product_data[$product_id];
            	}
        	}
