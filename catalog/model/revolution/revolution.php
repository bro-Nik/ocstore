...
class ModelRevolutionRevolution extends Model {
	...
	public function getReviewsByBlogId($revblog_id, $start = 0, $limit = 500) {
		$query = $this->db->query("SELECT rr.review_id, rr.parent_review_id, rr.author, rr.text, rr.text, rr.date_added FROM " . DB_PREFIX . "revblog_review rr WHERE rr.revblog_id = '" . (int)$revblog_id . "' AND rr.status = '1' AND rr.parent_review_id = '0' ORDER BY rr.date_added DESC LIMIT " . (int)$start . "," . (int)$limit);
		return $query->rows;
	}
	...
	public function getParentReviewsByBlogId($revblog_id, $review_id, $start = 0, $limit = 500) {
		$query = $this->db->query("SELECT rr.review_id, rr.parent_review_id, rr.author, rr.text, rr.text, rr.date_added FROM " . DB_PREFIX . "revblog_review rr WHERE rr.revblog_id = '" . (int)$revblog_id . "' AND rr.status = '1' AND rr.parent_review_id = '" . (int)$review_id . "' ORDER BY rr.date_added DESC LIMIT " . (int)$start . "," . (int)$limit);
		return $query->rows;
	}
