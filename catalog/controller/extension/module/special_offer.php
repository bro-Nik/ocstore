...
class ControllerExtensionModuleSpecialOffer extends Controller {
	public function index($setting) {
		...
		$data['text_timer_banner'] = $this->language->get('text_timer_banner');
		$data['text_before_start'] = $this->language->get('text_before_start');
		...
		if ($results) {
			foreach ($results as $result) {
				...
				if ($result['date_start'] != '0000-00-00') {
					...
					if (time() < strtotime($result['date_start'])) $timer_mode = 0;
				}
	...
	public function offerlist() {
		...
		$filter_special_offers['show_coming'] = true;
	...
	public function info() {
		...
		if ($special_offer_info) {
			...
			if ($special_offer_info['date_start'] == '0000-00-00') {
				...
				if (time() < strtotime($special_offer_info['date_start'])) $data['timer_mode'] = 0;
			}
			...
			$data['gift'] = $this->giftInfo($special_offer_info);
			...
			$results = $this->model_extension_module_special_offer->getProductsBySpecialOfferId($special_offer_id, $filter_data);
			$product_total = $this->model_extension_module_special_offer->getTotalProductsBySpecialOfferId($special_offer_id, $filter_data);
	...
	public function offerLabel($product) {
		...
		if (!empty($special_info['special_offer_id'])) {
			if(!empty($special_info['image_label'])) {
				...
				if (($special_info['offer_type'] == 1 || $special_info['offer_type'] == 5) && ($this->config->get('module_special_offer_giftphoto_as_label') == 1)) {
	...
	public function giftInfo($special_info) {
		$gift = array();
		if (($special_info['offer_type'] == 1 || $special_info['offer_type'] == 5) && (!empty($special_info['gift_product_id']))) {
			$gift_info = $this->model_catalog_product->getProduct($special_info['gift_product_id']);
			...
			$gift = array(
				'thumb'     => $gift_image,
				'name'      => $gift_info['name'],
				'href'      => $this->url->link('product/product', 'product_id=' . $special_info['gift_product_id'])
			);
		}
		return $gift;
	}
	public function GiftsInCart() {
		...
		foreach ($this->cart->getProducts() as $product) {
			...
			if (!empty($special_offer_id)) {
				$special_offer_info = $this->model_extension_module_special_offer->getSpecialOffer($special_offer_id);
				if ($special_offer_info['offer_type'] == 1  || $special_offer_info['offer_type'] == 5) {
					if (!empty($special_offer_info['gift_product_id'])) {
						if (array_key_exists($special_offer_id, $gift_offers)) {
							...
							$gift_offers[$special_offer_id]['sum'] += $product['total'];
						} else {
							$gift_offers[$special_offer_id]['qty'] = $product['quantity'];
							$gift_offers[$special_offer_id]['sum'] = $product['total'];
							...
							$gift_offers[$special_offer_id]['product_sum'] = $special_offer_info['product_sum'];
							...
							$gift_offers[$special_offer_id]['offer_type'] = $special_offer_info['offer_type'];
						}
					} else {
						...
						if ($gift_quantity>0) {
							...
							$gifts[] = array(
								...
								'price'     	=> $this->currency->format($this->config->get('module_special_offer_gift_price'), $this->session->data['currency']),
								'total'     	=> $this->currency->format($gift_quantity * $this->config->get('module_special_offer_gift_price'), $this->session->data['currency']),
								'weight'          => $product['weight'] * $gift_quantity,
		...
		foreach ($gift_offers as $gift_offer) {
			if ($gift_offer['offer_type'] == 1) {
				$gift_quantity=$gift_offer['gift_quantity']*intval($gift_offer['qty']/$gift_offer['product_quantity']);
			} elseif ($gift_offer['sum'] >= $gift_offer['product_sum']) {
				$gift_quantity=$gift_offer['gift_quantity'];
			}
			if ($gift_quantity>0) {
				...
				$gifts[] = array(
					...
					'price'     	=> $this->currency->format($this->config->get('module_special_offer_gift_price'), $this->session->data['currency']),
					'total'     	=> $this->currency->format($gift_quantity * $this->config->get('module_special_offer_gift_price'), $this->session->data['currency']),
					'weight'          => $gift_info['weight'] * $gift_quantity,
					'weight_class_id' => $gift_info['weight_class_id'],
					'length'          => $gift_info['length'],
					'width'           => $gift_info['width'],
					'height'          => $gift_info['height'],
					'length_class_id' => $gift_info['length_class_id'],
