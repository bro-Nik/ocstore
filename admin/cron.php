			...
			foreach($all as $r) {				
				if ($r['form_id'] == $f and $r['task'] == 4) {
					...
					if (!empty($r['link'])) {						
						...
						if ((!empty($r['link']) and !substr_count($r['link'], 'http') and !empty($r['ftp_name']) and !empty($r['ftp_pass'])) or (substr_count($r['link'], "ftp") and !empty($r['ftp_name']) and !empty($r['ftp_pass'])))  {
							$message ='';
							$flag = 1;
