...
if (isset($this->request->post['revtheme_home_blog'])) {
	...
	if (!$this->request->post['revtheme_home_blog']['slider_status']) {
		$tcss .= '#blog_mod.inhomeblogmod .blog-list .image {margin: 0 15px 0 0; float: left;}' . "\n";
		$tcss .= '#blog_mod.inhomeblogmod .blog-list .description {clear: none;}' . "\n";
		$tcss .= '#blog_mod.inhomeblogmod .col-lg-12 + .col-lg-12 .blog-list {border-top: none;}' . "\n";
	}
}
if (isset($this->request->post['revtheme_home_slideshow'])) {
	...
	if ($this->request->post['revtheme_home_slideshow']['allwide'] && $this->request->post['revtheme_home_slideshow']['video'] != '') {
		$tcss .= '#revslideshow.owl-carousel .owl-item, #revslideshow.owl-carousel .owl-item .carousel-item, #revslideshow.owl-carousel .owl-item .carousel-item video {width: 100% !important;}' . "\n";
	}
}
if (isset($this->request->post['revtheme_all_settings'])) {
	...
	if ($revtheme_all_settings['color_href_hover']) {
		$tcss .= 'a:hover,.list-group a:hover,.list-group a.active:hover,.pagination>li>a:hover,.pagination>li>span:hover,.top-links.verh a:hover,.top-links.verh .span-a:hover:hover,.product-info .btn-plus button:hover, .product-info .btn-minus button:hover,.product-info .pop_ico:hover, .product-info .compare .fa:hover, .product-info .wishlist .fa:hover,.product-info input[type="radio"]:checked + label span.option_price, .product-info input[type="checkbox"]:checked + label span.option_price,.home_block a:hover, .home_block a:hover .image > .fa,#menu2 ul.lastul li a:hover,#menu2 a:hover,#menu2 li.glavli a:hover,#menu .nav > li .dropdown-menu li .dropdown-menu ul li a:hover,#menu .nav > li > div > .dropdown-menu > div > ul > li > a:hover,.display-products-cart .btn-plus button:hover, .display-products-cart .btn-minus button:hover,.dropdown-menu li > a:hover, .dropdown-menu li > a:hover i,.search .search_button:hover,.search .change_category_button:hover,.popup_notification .mfp-close:hover,#popup-order-okno .mfp-close:hover, #popup-okno .mfp-close:hover,#popup-view-wrapper .mfp-close:hover,.wwicons a:hover,.cd-products-comparison-table .in-wishlist .fa-border, .cd-products-comparison-table .in-compare .fa-border,.cd-products-comparison-table .fa-border:hover,.product-info .in-wishlist .fa, .product-info .in-compare .fa,.well.well-sm .btn-group-justified .btn-default:hover,.dropdown-menu li > a:hover, .dropdown-menu li > a:hover i,.search .dropdown-menu > li > a:hover .name,.table-revolution .btn-plus button:hover, .table-revolution .btn-minus button:hover,.owl-carousel .owl-buttons div i:hover,.product-thumb .fa-border:hover, .product-info .pokupka .preimushestva .home_block a:hover .fa, .ul_block_home_catwalls h4.home_catwalls_all_href a:hover, .product-thumb .caption a.home_catwalls_podcat:hover,.product-thumb .in-wishlist .fa-border,.product-thumb .in-compare .fa-border,#menu2 ul.lastul .show_categoryoff:hover,.options_buy .checkbox label:hover,.options_buy .radio label:hover,.display-products-cart tr td.remove button:hover,#revcheckout .delete button:hover {color: #' . $revtheme_all_settings['color_href_hover'] . ';}' . "\n";
	...
	if ($revtheme_all_settings['cat_opis_opt']) {
		if ($revtheme_all_settings['cat_compact']) {
			$tcss .= '.product-grid .product-thumb:hover {position: absolute; margin: 1px 1px 0px 1px; top: 0; left: 0; transition: none;}' . "\n";
		} else {
			$tcss .= '.product-grid .product-thumb:hover {position: absolute; margin: 0px 15px; top: 0; left: 0; transition: none;}' . "\n";
		}
...
if (isset($this->request->post['revtheme_catalog_all'])) {
	...
	if ($setting_catalog_all['product_rating_counts']) {
		$tcss .= '.pr_reviews_count:hover, .pr_reviews_count_pr:hover {color: #'.$revtheme_all_settings['color_href_hover'].';}' . "\n";
	}
}
...
if (isset($this->request->post['revtheme_footer_all']['callbtn_color_href'])) {
	$tcss .= '.share_icon_callbtn.triggerbtn {background-color: #' . $this->request->post['revtheme_footer_all']['callbtn_color_backgr'] . '; color: #' . $this->request->post['revtheme_footer_all']['callbtn_color_href'] . '; box-shadow: 0 0 2px 2px #' . $this->request->post['revtheme_footer_all']['callbtn_color_backgr'] . ';}' . "\n";
}
if (isset($this->request->post['revtheme_dop_menu_callbtn'])) {
	if (!empty($this->request->post['revtheme_dop_menu_callbtn'])){
		$revtheme_dop_menus_callbtn = json_decode(htmlspecialchars_decode($this->request->post['revtheme_dop_menu_callbtn']), true);
		foreach($revtheme_dop_menus_callbtn as $key => $val){
			$tcss .= '.share_icon_callbtn.'.$revtheme_dop_menus_callbtn[$key]['sort'].' {background-color: #'.$revtheme_dop_menus_callbtn[$key]['color_backgr'].'; color: #'.$revtheme_dop_menus_callbtn[$key]['color_href'].';}' . "\n";
		}
	}
}
if ($this->request->post['revtheme_footer_user_set']['max_height']) {
	$tcss .= '#content .row.products_category, #content .row.rev_slider_cat_compact {display: flex !important; flex-wrap: wrap;}.product-layout.product-grid {display: flex;}, .product-layout.product-grid .product-thumb {display: flex; flex-direction: column;}.product-grid .product-thumb .caption {display: flex; flex-direction: column; flex: 1;}.product-grid .product-thumb .description_options {flex: 1;}' . "\n";
}
if ($this->request->post['revtheme_header_popupphone']['under_phone'] && !empty($this->request->post['revtheme_header_dop_contact'])) {
$tcss .= '#top2 .tel .s22:hover {cursor: default !important;}' . "\n";
if ($this->request->post['revtheme_header_phone'][$this->config->get('config_language_id')]['text'] != '' && $this->request->post['revtheme_header_phone'][$this->config->get('config_language_id')]['text2'] == '') {
$tcss .= '#top2 .tel .header_phone_image {vertical-align: top; padding-top: 15px;}' . "\n";
} else if ($this->request->post['revtheme_header_phone'][$this->config->get('config_language_id')]['text'] != '' && $this->request->post['revtheme_header_phone'][$this->config->get('config_language_id')]['text2'] != '') {
$tcss .= '#top2 .tel .header_phone_image {vertical-align: top; padding-top: 30px;}' . "\n";
} else {
$tcss .= '#top2 .tel .header_phone_image {vertical-align: top; padding-top: 0;}' . "\n";
}
}
if ($this->request->post['revtheme_header_phone'][$this->config->get('config_language_id')]['text'] != '' && $this->request->post['revtheme_header_phone'][$this->config->get('config_language_id')]['text2'] == '') {
$tcss .= '#top2 .search {margin-top: 22px;}#top2 #logo {margin-top: 8px;}' . "\n";
} else if ($this->request->post['revtheme_header_phone'][$this->config->get('config_language_id')]['text'] != '' && $this->request->post['revtheme_header_phone'][$this->config->get('config_language_id')]['text2'] != '') {
$tcss .= '#top2 .search {margin-top: 35px;}#top2 #logo {margin-top: 20px;}' . "\n";
}
//if ($this->request->post['config_customer_price']) {
//$tcss .= '.zatemnenie_img {-webkit-filter: initial; filter: initial; opacity: 1;}' . "\n";
//}
